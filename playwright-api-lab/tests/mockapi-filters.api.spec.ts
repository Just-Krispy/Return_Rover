...
// limit=N                           -> 200    at most N rows
// page=P&limit=L                    -> 200    1-indexed slice of L rows
...
async function clearExisting(request: any) {
  ...
  const res = await request.get(`${productsBaseUrl}?limit=100`);
  if (!res.ok()) return;
...
test.describe('MockAPI products: live-supported query mechanisms', () => {
  ...
  // NOTE: MockAPI free tier does NOT support real filtering. The `filter=` sign
  // operator 404s (paid plan) and the bracketed `filter[field]=value` form is accepted
  // (200) but returns every row unchanged. So this suite tests the mechanisms that
  // genuinely work against the live resource: search, sortBy, order, page, limit,
  // and reading each seeded row back. The two "filter" tests at the bottom assert
  // the REAL (non-filtering) behavior so we never assert on fantasy.

  test('seed the catalog (deterministic 5 rows)', async ({ request }) => {
  ...
  test('limit caps the number of rows', async ({ request }) => {
    const { status, rows } = await getJson(request, 'limit=2');
    expect(status).toBe(200);
    expect(rows.length).toBe(2);
  });

  test('pagination slices rows (1-indexed page)', async ({ request }) => {
    const { status, rows } = await getJson(request, 'page=2&limit=2');
    expect(status).toBe(200);
    expect(rows.length).toBe(2);
    // page 2 of limit 2 == rows 3 & 4 of the seeded catalog.
    expect(rows[0].name).toBe('Dr. Circuit Book 3');
  });
  ...
  test('cleanup seeded rows', async ({ request }) => {
    await clearExisting(request);
    const res = await request.get(`${productsBaseUrl}?limit=100`);
